...
MCP 的配置目录在 `/sdcard/Download/Operit/mcp_plugins/` 这个路径下。主要的配置文件是 `mcp_config.json`，放在这个目录里。你可能会注意到还有一个 `server_status.json`，那个只是状态缓存，不是配置用的，不用管它。

`mcp_config.json` 的顶层结构是这样的：有一个 `mcpServers` 对象，里面的每个 key 就是一个 MCP 插件的 ID。每个插件用以下字段配置：

- `command`：启动命令，必填。比如 `node`、`python`、`npx`、`uvx` 这些。
- `args`：命令参数，数组形式。比如 `["dist/index.js"]`。
- `env`：环境变量，键值对形式。比如 API Key 之类的敏感信息写在这里。
