...
So the pattern is: built-in sandbox package descriptions are hardcoded in the app (external ones come from the script itself), MCP descriptions are generated by the function model after startup, and Skill descriptions — you write them yourself. This is a key difference between Skill and the other tool types.
