...
本章描述的是 Operit 源码仓库中的材料布局，适合源码贡献者和使用桌面开发环境的插件作者。应用内 AI 协作开发不需要先克隆这个仓库：它应先更新 `SandboxPackage_DEV`，并在 `/sdcard/Download/Operit/dev_package/` 下创建插件项目。
...
### `tools/` 是源码仓库中的运行与调试入口

写完代码以后，最终还是要跑起来。这些脚本依赖本机的 ADB 与源码仓库环境，不是应用内 Skill 开发流程的一部分。
