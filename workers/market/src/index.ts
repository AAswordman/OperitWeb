...
import { MarketError, corsHeaders, fail, jsonResponse, withHeaders } from './shared.js';
...
async function routeV2(pathname: string, request: Request, env: MarketEnv): Promise<Response | JsonObject> {
  ...
  if (pathname.includes('/entries/') && pathname.endsWith('/comments') && request.method === 'POST') return interact.addComment(request, storeEnv);
  if (pathname.startsWith('/market/v2/comments/') && request.method === 'PATCH') return interact.editComment(request, storeEnv);
  if (pathname.startsWith('/market/v2/comments/') && request.method === 'DELETE') return interact.deleteComment(request, storeEnv);
  if (pathname.includes('/entries/') && pathname.endsWith('/reactions') && request.method === 'POST') return interact.reactToEntry(request, storeEnv);
