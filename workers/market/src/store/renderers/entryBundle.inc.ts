...
export async function buildEntryItem(d1: RendererContext['d1'], entry: Row): Promise<EntryItem> {
  ...
  item.versions = await Promise.all(versions.map(async (version: Row) => {
    ...
    return {
      ...
      stateCode: rowText(version, 'state_code'),
  ...
  item.latestVersion = { id: rowText(latest, 'id'), version: rowText(latest, 'version'), formatVer: rowText(latest, 'format_ver'), minAppVer: rowText(latest, 'min_app_ver'), ...(rowOptionalText(latest, 'publisher_id') ? { publisherId: rowText(latest, 'publisher_id') } : {}), ...(latestPublisher ? { publisher: { id: rowText(latest, 'publisher_id'), login: rowText(latestPublisher, 'github_login'), avatar: rowText(latestPublisher, 'owner_avatar') } } : {}), ...(rowOptionalText(latest, 'max_app_ver') ? { maxAppVer: rowText(latest, 'max_app_ver') } : {}), ...(rowOptionalText(latest, 'api_version') ? { apiVersion: rowText(latest, 'api_version') } : {}), ...(rowOptionalText(latest, 'changelog') ? { changelog: rowText(latest, 'changelog') } : {}), ...(repoVersion && rowOptionalText(repoVersion, 'install_config') ? { installConfig: rowText(repoVersion, 'install_config') } : {}), ...(rowOptionalText(latest, 'runtime_pkg') ? { runtimePackageId: rowText(latest, 'runtime_pkg') } : {}), stateCode: rowText(latest, 'state_code'), ...(rowOptionalText(latest, 'published_at') ? { publishedAt: rowText(latest, 'published_at') } : {}) };
  if (repoVersion) item.repoVersion = { refType: rowText(repoVersion, 'ref_type'), refName: rowText(repoVersion, 'ref_name'), ...(rowOptionalText(repoVersion, 'install_config') ? { installConfig: rowText(repoVersion, 'install_config') } : {}) };
