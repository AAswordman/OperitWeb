...
const SORTS = ['updated', 'likes', 'featured'] as const;
...
export async function fullBuildIfNeeded(env: MarketEnv): Promise<{ ok: true; materialized: number } | { ok: true; skipped: true }> {
  const store = requireStore(env);
  const meta = await store.d1.getMeta('last_full_build');
  ...
  await store.d1.setMeta('last_full_build', String(now));
  return result;
}

export async function incrementalBuild(env: MarketEnv): Promise<{ ok: true; materialized: number }> {
  const store = requireStore(env);
  const dirtyRows = await store.d1.listDirty(200);
  ...
  for (const row of dirtyRows) {
    ...
    if (projection === 'entry.versions' && scope.entryId) {
      ...
      await store.d1.deleteDirty(projection as ProjectionPlan['projection'], scopeKey);
    }
  }
  ...
  for (const entryId of entryIds) {
    ...
    await store.d1.deleteDirty('entry.versions', registry.scopeKeyOf({ entryId }));
  }
  ...
  for (const { plan, scopeKey } of directPlans) {
    await store.materialize(plan);
    await store.d1.deleteDirty(plan.projection, scopeKey);
    count++;
  }

  if (listDirty.length > 0) {
    const snap = await store.d1.loadBuildSnapshot();
    count += await buildAllListPages(snap, createBuildSnapshotIndex(snap), r2, registry);
...
async function fullBuild(store: MarketStore): Promise<{ ok: true; materialized: number }> {
  ...
  await r2.writeJson(registry.keyOf('manifest', {}), {
    ...
    formatVersions: snap.formatVersions.map((f) => ({
      ...
      publishable: rowBool(f, 'publishable'), legacyImportable: rowBool(f, 'legacy_importable'),
    })),
  ...
  const entryShards = new Map<string, Record<string, unknown>>();
  for (let i = 0; i < ENTRY_SHARDS; i++) entryShards.set(i.toString(16).padStart(2, '0'), {});
  for (const entry of snap.entries.filter((e) => rowText(e, 'state_code') === 'approved')) {
    const item = buildEntryFromSnapshot(entry, snap, index);
    const shard = entryShardOf(item.id);
    entryShards.get(shard)![item.id] = item;
  }
  let entryW = 0, entryS = 0;
  for (const [shard, entriesById] of entryShards) {
    if (Object.keys(entriesById).length === 0) { entryS++; continue; }
  ...
  const publisherShards = new Map<string, Array<{ id: string; title: string; stateCode: string; updatedAt: string }>>();
  ...
  for (const entry of snap.entries) {
    ...
    publisherShards.get(shard)!.push({ id: rowText(entry, 'id'), title: rowText(entry, 'title'), stateCode: rowText(entry, 'state_code'), updatedAt: rowText(entry, 'updated_at') });
  }
  let pubW = 0, pubS = 0;
  for (const [shard, entries] of publisherShards) {
    if (entries.length === 0) { pubS++; continue; }
    await r2.writeJson(registry.keyOf('private.publisherShard', { shard, authorId: '' }), {
      ok: true, marketVersion: 2, generatedAt: isoNow(), shard, entries,
    });
    pubW++; count++;
  }
...
async function buildAllListPages(snap: BuildSnapshot, index: ReturnType<typeof createBuildSnapshotIndex>, r2: MarketStore['r2'], registry: MarketStore['projectionRegistry']): Promise<number> {
  const pageSize = 50;
  let count = 0;
  const reactionTotals = reactionTotalsByEntry(snap);
  const featuredPositions = featuredPositionsByEntry(snap);
  ...
  const sortFns: Record<SortKey, (a: Row, b: Row) => number> = {
    ...
    featured: (a, b) => {
      const aP = featuredPositions.get(rowText(a, 'id')) ?? 9999;
      const bP = featuredPositions.get(rowText(b, 'id')) ?? 9999;
      return aP - bP || rowText(b, 'updated_at').localeCompare(rowText(a, 'updated_at'));
    },
  };
...
async function rebuildEntry(env: MarketEnv, entryId: string): Promise<{ ok: true; materialized: number }> {
  ...
  const plans: ProjectionPlan[] = [
    { projection: 'entry.shard', scope: { shard } },
...
function reactionTotalsByEntry(snap: BuildSnapshot): Map<string, number> {
  const result = new Map<string, number>();
  for (const row of snap.reactions) {
    if (rowText(row, 'reaction') !== '+1') continue;
    const entryId = rowText(row, 'entry_id');
    result.set(entryId, (result.get(entryId) ?? 0) + Number(row.total_count || 0));
  }
  return result;
}

function featuredPositionsByEntry(snap: BuildSnapshot): Map<string, number> {
  const result = new Map<string, number>();
  for (const row of snap.curations) {
    if (rowText(row, 'list_key') !== 'featured') continue;
    result.set(rowText(row, 'entry_id'), Number(row.position ?? 9999));
  }
  return result;
}

function snapshotSummary(snap: BuildSnapshot): string {
  return `entries=${snap.entries.length} versions=${snap.versions.length} repos=${snap.repos.length} assets=${snap.assets.length} reactions=${snap.reactions.length} categories=${snap.categories.length} curations=${snap.curations.length}`;
}
